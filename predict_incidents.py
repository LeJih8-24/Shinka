...
def predict_incidents(csv_path, station_depart):
    ...
    df = df[df["Departure station"] == station_depart]
    if df.empty:
        print(f"Aucun trajet trouvé depuis la station : {station_depart}")
        return
    ...
    print(f"\n🧾 Analyse pour la station de départ : {station_depart}\n")
    print("🔎 Prédiction d'incident (0 = non, 1 = oui)")
    print(classification_report(y_test_i, y_pred_i))

    print("\n🔍 Prédiction de la cause principale de l'incident")
    print(classification_report(y_test_c, y_pred_c))

    # Graph cause principale prédite
    cause_counts = pd.Series(y_pred_c).value_counts()
    sns.barplot(x=cause_counts.index, y=cause_counts.values)
    plt.xticks(rotation=45, ha="right")
    plt.title(f"Causes prédites des incidents - {station_depart}")
...
if __name__ == "__main__":
    predict_incidents("cleaned_dataset.csv")
