...
def station_date():
    ...
    with years_col:
        year = st.selectbox(
            language_dic[st.session_state["language"]]["select_year"],
            years,
            index=len(years) - 1,
        )
    with month_col:
        month = st.selectbox(
            language_dic[st.session_state["language"]]["select_month"],
            language_dic[st.session_state["language"]]["months"],
        )
    ...
    if "error" in result:
        ...
        st.markdown(
            f"### {language_dic[st.session_state['language']]['data_for_months']} **:orange[{month} {year}]**"
        )
        ...
        with one:
            st.markdown(
                f"#### {language_dic[st.session_state['language']]['pct_cancellation']}"
            )
