...
def station_page():
    ...
    option = st.selectbox(
        language_dic[st.session_state["language"]]["select_station"], df["Cities"]
    )

    st.markdown("---")
    st.subheader(
        f"{language_dic[st.session_state['language']]['data_for_station']}**{option}**"
    )
    ...
    with average_delay:
        with st.container():
            st.markdown(
                f"#### {language_dic[st.session_state['language']]['avg_delay']}"
            )
            sv.gauge(
                dic["Average delay"],
                gSize="MED",
    ...
    with average_journey_time:
        with st.container():
            st.markdown(
                f"#### {language_dic[st.session_state['language']]['avg_journey_time']}"
            )
            sv.gauge(
