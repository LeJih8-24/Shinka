...
def get_ratio_national(df, station_name):
    len_national = len(
        df[(df["Departure station"] == station_name) & (df["Service"] == "National")]
    )
    len_international = len(
        df[
            (df["Departure station"] == station_name)
            & (df["Service"] == "International")
        ]
    )
    if len_national == 0:
        return (0, 100)
...
def get_biggest_delay_cause(df):
    ...
    causes_names = [
        ...
        "rollign stock issues",
        "station management issues",
        "person management issues",
    ]
    causes = [
        external_causes,
        infrastructure_causes,
        traffic_management_causes,
        rolling_stock_causes,
        station_management_causes,
        person_management_causes,
    ]
...
def get_values_per_station(df: pd.DataFrame, station_name):
    ...
    mean_delay = get_mean_value_df(
        station_line, "Average delay of all trains at departure"
    )
...
def get_route_info(start, end, df: pd.DataFrame):
    ...
    mean_delay = get_mean_value_df(
        station_lines, "Average delay of all trains at departure"
    )
...
def extract_monthly_metrics(df, year, month):
    ...
    if filtered_df.empty:
        return {
            "error": f"{language_dic[st.session_state['language']]['no_data_date']} {year}-{str(month).zfill(2)}"
        }

    result = {
        "Total scheduled trains": int(
            get_sum_value_df(filtered_df, "Number of scheduled trains")
        ),
        "Cancelled trains": int(
            get_sum_value_df(filtered_df, "Number of cancelled trains")
        ),
        ...
        "Average delay of all arrivals (min)": round(
            get_mean_value_df(filtered_df, "Average delay of all trains at arrival"), 2
        ),
    }
...
def top_scheduled_departure_stations(df: pd.DataFrame, top_n: int = 3) -> pd.DataFrame:
    if (
        "Departure station" not in df.columns
        or "Number of scheduled trains" not in df.columns
    ):
    ...
    df_clean = df_clean.dropna(
        subset=["Departure station", "Number of scheduled trains"]
    )

    scheduled_counts = (
        df_clean.groupby("Departure station", as_index=False)[
...
def top_cancelled_departure_stations(df: pd.DataFrame, top_n: int = 3) -> pd.DataFrame:
    ...
    if (
        "Departure station" not in df_clean.columns
        or "Number of cancelled trains" not in df_clean.columns
    ):
        return pd.DataFrame({"error": ["Colonnes manquantes dans la dataframe"]})
    ...
    df_clean = df_clean.dropna(
        subset=["Departure station", "Number of cancelled trains"]
    )
...
def top_avg_journey_time_stations(df: pd.DataFrame, top_n: int = 3) -> pd.DataFrame:
    if (
        "Departure station" not in df.columns
        or "Average journey time" not in df.columns
    ):
        return pd.DataFrame({"error": ["Colonnes manquantes dans la dataframe"]})

    df_clean = df.copy()

    df_clean["Average journey time"] = pd.to_numeric(
        df_clean["Average journey time"], errors="coerce"
    )
