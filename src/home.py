...
def home_page():
    ...
    with train_sched:
        display_bar_chart(
            dic["Top 3 stations with most trains scheduled"],
            language_dic[st.session_state["language"]]["train_scheduled"],
            "Total scheduled trains",
        )
    with train_canc:
        display_bar_chart(
            dic["Top 3 stations with most trains cancelled"],
            language_dic[st.session_state["language"]]["train_cancelled"],
            "Total cancelled trains",
        )
    with avg_journey_time:
        display_bar_chart(
            dic["Top 3 stations with most average journey time"],
            language_dic[st.session_state["language"]]["avg_journey_time"],
    ...
    with col1:
        st.metric(
            language_dic[st.session_state["language"]]["avg_journey_time(hours)"],
            value=f"{round(dic['Average journey time'] / 60, 1)}h",
        )
    with col2:
        st.metric(
            language_dic[st.session_state["language"]]["avg_delay_time(minutes)"],
            value=f"{dic['Average delay']}m",
        )
    with col3:
        st.metric(
            language_dic[st.session_state["language"]]["next_month_prediction"],
            value=f"{pred['predict mean']}m",
        )

    st.subheader(language_dic[st.session_state["language"]]["top_delay_cause"])
    st.markdown(
        f'<div class="stInfo">Main cause of delay: <strong>{dic["Biggest delay cause"][0]}</strong></div>',
        unsafe_allow_html=True,
    )
